    ...
    public int onEnd() {
        if (wakeupBeh.done()) {
            if (receiveBeh.onEnd() == 1) {
                ...
                this.myAgent.addBehaviour(new Success());
                return 1;
            } else if (receiveBeh.onEnd() == 2) {
                log.info("Обработка отказа...");
                this.myAgent.addBehaviour(new Fail());
                return 2;
            } else {
                log.info("Отправка повторного запроса");
                this.myAgent.addBehaviour(new OrderBeh());
                this.myAgent.addBehaviour(new ParallelProcessingBeh());
                return 0;
            }
        } else {
            if (receiveBeh.onEnd() == 1) {
